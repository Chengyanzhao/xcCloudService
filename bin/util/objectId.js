...
function ObjectId(bytes) {
    ...
    } else if (typeof bytes == 'number' || Object.prototype.toString.call(bytes) == '[object Date]') {
        this.bytes = _objectId(+bytes)
    } else if (typeof bytes !== 'undefined') {
        throw new Error("Unrecognized type: " + bytes)
    } else {
        this.bytes = _objectId(Date.now())
    }

    function _objectId(time) {
